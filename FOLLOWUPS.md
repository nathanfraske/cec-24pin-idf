...
| C7 | ✅ guard added (saturation PR) | `main.c` main loop | **Prototype 5V/0x41 INA226 died on the bench.** A bad `V+→IN+` shunt-jumper joint bridged to ground, ran away thermally, and cooked the die. Symptom looked like a plausible reading: current pinned at full scale (8.19 A = ±81.92 mV / 10 mΩ) dead flat — including in HS — while `v_5v` stayed ~4.8 V. The software current path has no IMAX clamp, so a saturated shunt register just reports the ceiling. Added a per-rail **saturation watchdog**: a rail pinned at ≥`SAT_PIN_FRACTION`·IMAX for ~1 s logs `SATURATION on <rail>` (sustained over-current *or* railed/faulty sensor), re-warns ~5 s, logs recovery on clear. Fed only fresh reads (freezes on capture-skip/NACK). **Optional follow-up:** latch a persistent "rail N sense fault — replace module" flag into `status` so the condition survives the scrolling log. Replacement INA226 must be strapped to 0x41 (A0→VS, A1→GND). |
